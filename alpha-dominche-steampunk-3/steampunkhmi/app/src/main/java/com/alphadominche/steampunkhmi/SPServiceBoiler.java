...
public class SPServiceBoiler extends Thread {
    ...
    public final double NEXT_WARNING_OFFSET = 0.5;
    public final int MAX_OVERHEAT_LIMIT = 10;
    ...
    private double mOverheatWarningLimit;
    ...
    private static OverheatingListener mOverheatingListener;
    ...
    SPServiceBoiler(
        ...
        super();
        SPLog.debug("############# instantiating boiler");
        mInManualMode = false;
        ...
        mThermistor = new SPServiceThermistor(temperatureSignal, tempType, service);
        mThermistor.boiler = true;
        ...
        mOverheatWarningLimit = mMaxTemperature + MAX_OVERHEAT_LIMIT;
        mSignal = false;
        mForceHeatOn = false;
        mForceHeatOff = false;
        mService = service;
    ...
    public void run() {
        try {
            while (mRunning) {
                ...
                if (this.getTemperature() > mOverheatWarningLimit) {
                    mOverheatWarningLimit += NEXT_WARNING_OFFSET;
                    if (mOverheatingListener != null) {
                        mOverheatingListener.overheating();
                    }
                }
            ...
            SPLog.debug("boiler exiting due to exception: " + e.toString());
    ...
    public static void registerOverheatingListener(OverheatingListener listener) {
        mOverheatingListener = listener;
    }

    public static void unregisterOverheatingListener() {
        mOverheatingListener = null;
    }

    public interface OverheatingListener {
