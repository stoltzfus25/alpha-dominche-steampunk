...
//		}
	...
	public void bindView(View convertView, Context context, Cursor cursor) { //SPLog.debug("binding view...");
		...
		recipeNameView.setText(cursor.getString(Name_index)); SPLog.debug("binding view..." + cursor.getString(Name_index));

		long ID = cursor.getLong(ID_index);
		selected.setImageResource(R.drawable.teal_circle);
		selected.setVisibility(View.VISIBLE);
		if (mSelectedItem == ID) { SPLog.debug("this item is getting shown as selected!");
			selected.setVisibility(View.VISIBLE);
		} else {
