...
//	private SPRecipeAgSliderInfo mInfo;
	...
	public void setDurationResponder(SPSliderModalResponder responder, int max, int progress) {
		...
		mLimitingTotal = Math.max(mStartMax + wholeNumberValue(mDurationSeekBarProgress / 10.0), wholeNumberValue(mDurationMax)); SPLog.debug("SET LIMITING TOTAL: " + mLimitingTotal + " setMax: " + max);
	}
...
//		setPulseWidthResponder(null, mInfo.getPulseWidthProgressMax(), mInfo.getPulseWidthProgress());
		View group = mView.findViewById(R.id.ag_duration_slider_group);
		SeekBar sb = (SeekBar)group.findViewById(R.id.seek_bar);
		if (!mIsFirstAg) mStartMax = mLimitingTotal - wholeNumberValue(sb.getProgress() / 10.0); SPLog.debug("limiting total: " + mLimitingTotal + " startMax: " + mStartMax + " duration: " + wholeNumberValue(sb.getProgress() / 10.0) + "(" + sb.getProgress() + ")");
		group = mView.findViewById(R.id.ag_start_slider_group);
		sb = (SeekBar)group.findViewById(R.id.seek_bar);
		sb.setMax(mStartMax - mStartMin);SPLog.debug("start max: " + sb.getMax());
