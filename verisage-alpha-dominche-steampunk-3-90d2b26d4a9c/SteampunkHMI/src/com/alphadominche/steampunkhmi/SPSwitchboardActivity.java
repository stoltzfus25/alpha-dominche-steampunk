...
//		mBoilerFillIndicator = fincViewById(R.id.boiler_fill_indicator); // TODO need this put into the design
		...
		mBoilerHeatingElementBtn.setOnClickListener(new OnClickListener() {
			@Override
	...
	private void toggleBoilerHeat() {
		SPModel model = SPModel.getInstance(this);
...
//				}
	...
	public void notifyOfConnectionStatus() {
		...
		runOnUiThread(new Runnable() {
			@Override
			public void run() { SPLog.debug("got connection notification");
				if (!mActive) return;
