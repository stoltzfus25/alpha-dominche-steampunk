...
public class SPRecipeAgitation extends Observable {
	...
	public void setLength(double length) { SPLog.debug("set length!" + "stack..."); //StackTraceElement[] e = (new Exception()).getStackTrace(); for(int i = 0;i < e.length;i++) SPLog.debug("line: " + e[i]);
		mLength = length;
		setChanged();
		notifyObservers();
	}
